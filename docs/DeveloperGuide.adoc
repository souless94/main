...
* `list [index]` -- lists all the groups the person at [index] is currently in.
...
[MelodiesTODO: Elaborate on how connection to UI is created.]

.Sequence Diagram of RegisterCommand (Logic component)
image::RegisterCommandLogicSequenceDiagram.png[width="750"]


.Sequence Diagram of RegisterCommand (Model component)
image::RegisterCommandModelSequenceDiagram.png[width="750"]
...
image::FindFeature1.png[width="300"]

==== Design Considerations

Aspect: How should each specific 'find' feature work

*Alternative 1 (Current Choice) :* Create a class for each information (e.g. find_address)

Pros: Easy to implement, with specific search focus

Cons: Repetitive code base with less flexibility

*Alternative 2 :* Use prefixes for specific search (e.g. find a/gardens)

Pros: Higher flexibility

Cons: Harder to implement. May allow greater control for larger search range
