...
* *Minor enhancement*: Created an Abstract class Entity which Group and Person extends from to allow for polymorphism: Refactored the ModelManager and AddressBook function to include Polymorphism. 
E.g. addPerson() --> add() which performs add on Group or Person class based on polymorphism.
