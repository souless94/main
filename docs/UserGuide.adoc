...
add a timetable to the person specified by `INDEX` from the stored folder of the person specified by `INDEX` if no file location is present. Adds timetable to the person specified by the `INDEX` from file location if file location is present.Will not be able to add if timetable is not present in the stored location of person specified by `INDEX` Existing values will be overwritten by the input values. +
